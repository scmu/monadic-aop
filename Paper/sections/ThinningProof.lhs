...
\begin{spec}
     (thin_preceq . (f x <=< mem)) =<< (thin_preceq . foldR f e) xs `sse`
       (thin_preceq . foldR f e) (x : xs)
<=>      {- definition of |foldR| -}
     (thin_preceq . (f x <=< mem)) =<< (thin_preceq . foldR f e) xs `sse`
       thin_preceq (f x =<< foldR f e xs)
<=>     {- abstracting away |xs| -}
     (thin_preceq . (f x <=< mem)) <=< (thin_preceq . foldR f e) `sse`
       thin_preceq . (f x <=< foldR f e) {-"~~."-}
\end{spec}
%if False
\begin{code}
propFixPoint0 :: (a -> b -> P b) -> P b -> a -> List a -> P (T b)
propFixPoint0 f e x xs =
  (thin_preceq . (f x <=< mem)) =<< (thin_preceq . foldR f e) xs
  `sse` (thin_preceq . foldR f e) (x : xs)

propFixPoint1 :: (a -> b -> P b) -> P b -> a -> List a -> P (T b)
propFixPoint1 f e x xs =
  (thin_preceq . (f x <=< mem)) =<< (thin_preceq . foldR f e) xs
  `sse` (thin_preceq . foldR f e) (x : xs)
  `sse` thin_preceq (f x =<< foldR f e xs)

propFixPoint2 :: (a -> b -> P b) -> P b -> a -> List a -> P (T b)
propFixPoint2 f e x  =
  (thin_preceq . (f x <=< mem)) <=< (thin_preceq . foldR f e)
  `sse` thin_preceq . (f x <=< foldR f e)
\end{code}
%endif
...
%if False
\begin{code}
pfThinThm2 :: ((b, b) -> Bool) -> (a -> b -> P b) -> P b -> a -> P (T b, b)
pfThinThm2 preceq f e x =
\end{code}
%endif
%format `fpreceq` = "\preceq"
\begin{code}
      do  xs <- any
...
%if False
\begin{code}
  where fpreceq b1 b0 = filt preceq (b1, b0)
\end{code}
%endif
%}
