...
\setlength{\jot}{-1pt}
 ...
 \end{aligned}\mbox{~~.}
\end{equation*}
When proving an inequality, it is usually easier to start from the smaller side (the left-hand side in this case) and stepwise relaxing to to a larger value (the right-hand side).
...
To apply the law, notice that lines generating |xs|, |b1|, and |b0| match that of the left-hand side of \eqref{eq:max-cancelation}, therefore we rewrite them accordingly to the right-hand side:
\begin{spec}
       ...
...
We massage the expression such that some parts (lines) of it matches the left-hand sides of known laws such as |max|-cancelation \eqref{eq:max-cancelation} or monotonicity \eqref{eq:monotonicity}.
...
%        filt rhd (b0, b1) {-"~~."-}
% \end{spec}
The first step can be carried out by |max|-cancelation with |f := id|. The rest is the same as the proof presented above.
