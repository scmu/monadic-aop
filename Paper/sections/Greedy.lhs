...
Notice that, while the pairs |(y1, z0)| in the righthand side are generated by |any|,
...
\setlength{\jot}{-1pt}
 ...
 \end{aligned}\mbox{~~.}
\end{equation*}
When proving an inequality, it is usually easier to start from the smaller side (the lefthand side in this case) and stepwise relaxing to to a larger value (the righthand side).
That is, given the lefthand side we wish to show that |y1 `unrhd` y0|, holds.
...
To make use of monotonicity, notice that the lines generating |(b1, b0)| and |y0| and the |b1 `unrhd` b0| match the lefthand side of \eqref{eq:monotonicity}, therefore we rewrite them to the righthand side of \eqref{eq:monotonicity}:
\begin{spec}
       ...
