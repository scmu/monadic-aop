...
module Examples.Knapsack.Impl_v2 where
...
open Thinning using (T; mem; mergeT)
...
-- `add w x t` extends every candidate in t with x, keeping those still within
-- the capacity.  It is an ordinary list function; its set-level meaning is
-- `mem-add` below.
add : Wgt → Item → T (List Item) → T (List Item)
add w x []       = []
add w x (ys ∷ t) = if withinW w (x ∷ ys) then (x ∷ ys) ∷ add w x t else add w x t
...
-- The two clauses of `add`, as equations.
add-[] : ∀ w x → add w x [] ≡ []
add-[] w x = refl
...
add-∷-true w x ys t eq =
  cong (λ b → if b then (x ∷ ys) ∷ add w x t else add w x t) eq
...
-- The specification of `add`: its members are exactly the candidates obtained
-- by consing x onto a member of t and filtering by capacity.
mem-add : ∀ w x t → mem (add w x t) ≡ ((filt (withinW w) ∘ (_∷_ x)) =<< mem t)
mem-add w x [] = sym (=<<-∅ (filt (withinW w) ∘ (_∷_ x)))
mem-add w x (ys ∷ t) =
  bool-split (withinW w (x ∷ ys)) (mem (add w x (ys ∷ t)) ≡ (f =<< mem (ys ∷ t)))
    (λ eq →
      mem (add w x (ys ∷ t))
        ≡⟨ cong mem (add-∷-true w x ys t eq) ⟩
      (return (x ∷ ys) ∪ mem (add w x t))
        ≡⟨ cong₂ _∪_ (sym (filt-true (withinW w) (x ∷ ys) eq)) (mem-add w x t) ⟩
      (f ys ∪ (f =<< mem t))
        ≡⟨ sym (split-∷) ⟩
      (f =<< mem (ys ∷ t))
        ∎)
    (λ eq →
      mem (add w x (ys ∷ t))
        ≡⟨ cong mem (add-∷-false w x ys t eq) ⟩
      mem (add w x t)
        ≡⟨ mem-add w x t ⟩
      (f =<< mem t)
        ≡⟨ sym (∪-∅-unit-l (f =<< mem t)) ⟩
      (∅ ∪ (f =<< mem t))
        ≡⟨ cong (_∪ (f =<< mem t)) (sym (filt-false (withinW w) (x ∷ ys) eq)) ⟩
      (f ys ∪ (f =<< mem t))
        ≡⟨ sym (split-∷) ⟩
      (f =<< mem (ys ∷ t))
        ∎)
  where
    f = filt (withinW w) ∘ (_∷_ x)

    -- f =<< (return ys ∪ mem t) splits as f ys ∪ (f =<< mem t)
    split-∷ : (f =<< mem (ys ∷ t)) ≡ (f ys ∪ (f =<< mem t))
    split-∷ = =<<-∪-dist-left f (return ys) (mem t)
