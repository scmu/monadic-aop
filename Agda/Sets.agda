...
open import Cubical.Data.Empty using (isProp⊥* ; ⊥* ; elim*)
...
∪-∩-dist-r : (A B C : ℙ X) → (A ∪ (B ∩ C)) ⊆ ((A ∪ B) ∩ (A ∪ C))
∪-∩-dist-r A B C x x∈L =
  rec (P.∈-isProp ((A ∪ B) ∩ (A ∪ C)) x)
      (λ { (_⊎_.inl x∈A)         → ∣ _⊎_.inl x∈A ∣₁ , ∣ _⊎_.inl x∈A ∣₁
         ; (_⊎_.inr (x∈B , x∈C)) → ∣ _⊎_.inr x∈B ∣₁ , ∣ _⊎_.inr x∈C ∣₁ })
      x∈L

∪-∩-dist-l : (A B C : ℙ X) → ((A ∪ B) ∩ (A ∪ C)) ⊆ (A ∪ (B ∩ C))
∪-∩-dist-l A B C x x∈R =
  rec squash₁
      (λ { (_⊎_.inl x∈A) → ∣ _⊎_.inl x∈A ∣₁
         ; (_⊎_.inr x∈B) →
             rec squash₁
                 (λ { (_⊎_.inl x∈A) → ∣ _⊎_.inl x∈A ∣₁
                    ; (_⊎_.inr x∈C) → ∣ _⊎_.inr (x∈B , x∈C) ∣₁ })
                 (x∈R .snd) })
      (x∈R .fst)

∪-∩-dist : (A B C : ℙ X) → A ∪ (B ∩ C) ≡ (A ∪ B) ∩ (A ∪ C)
∪-∩-dist A B C =
  P.⊆-extensionality (A ∪ (B ∩ C)) ((A ∪ B) ∩ (A ∪ C))
    (∪-∩-dist-r A B C , ∪-∩-dist-l A B C)

antitone-∪⊆∪ : (A B : ℙ X) → (f : ℙ X → ℙ X) → (∀ Q R → Q ⊆ R → f R ⊆ f Q) → f (A ∪ B) ⊆ (f A ∪ f B)
antitone-∪⊆∪ A B f p = λ x z → ∣ _⊎_.inl (p A (A ∪  B) (λ x₁ z₁ → ∣ _⊎_.inl z₁ ∣₁) x z) ∣₁ 